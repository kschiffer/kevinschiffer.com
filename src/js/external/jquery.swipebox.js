				...
				'<div id="swipebox-overlay">\
					<div id="swipebox-container">\
						...
						<div id="swipebox-bottom-bar">\
							<p class="info">Use gestures to zoom and pan</p>\
						</div>\
				...
				$('body')
					.bind('touchstart', function(event) {
						...
						if (event.originalEvent.targetTouches.length > 1) {
							...
							pinchData.startDistance = Math.sqrt(
								Math.pow(
									event.originalEvent.targetTouches[0].pageX -
										event.originalEvent.targetTouches[1].pageX,
									2
								) +
									Math.pow(
										event.originalEvent.targetTouches[0].pageY -
											event.originalEvent.targetTouches[1].pageY,
										2
									)
							);
						...
						$('.touching').bind('touchmove', function(event) {
							...
							if (pinchData.inProgress) {
								...
								var perc =
									pinchData.startZoom *
									(1 +
										(lastDistance - pinchData.startDistance) /
											pinchData.startDistance);
								...
								pinchData.diffHeight = Math.max(
									0,
									$('#swipebox-slider .current img').height() -
										$('#swipebox-slider .current').height()
								);
								pinchData.diffWidth = Math.max(
									0,
									$('#swipebox-slider .current img').width() -
										$('#swipebox-slider .current').width()
								);

								pinchData.overZoom = false;
								if (pinchData.diffHeight > 0) {
									pinchData.marginTop +=
										-(pinchData.diffHeight - nowDiffHeight) / 2;
									pinchData.marginTop = Math.min(0, pinchData.marginTop);
									pinchData.marginTop = Math.max(
								...
								if (pinchData.diffWidth > 0) {
									pinchData.marginLeft +=
										-(pinchData.diffWidth - nowDiffWidth) / 2;
									pinchData.marginLeft = Math.min(0, pinchData.marginLeft);
									pinchData.marginLeft = Math.max(
										-pinchData.diffWidth,
										pinchData.marginLeft
									);
									pinchData.overZoom = true;
								...
								if (
									nowDiffHeight == pinchData.diffHeight &&
									nowDiffWidth == pinchData.diffWidth
								) {
								...
								pinchData.lastMarginTop =
									pinchData.marginTop + endCoords.pageY - startCoords.pageY;
								pinchData.lastMarginTop = Math.min(0, pinchData.lastMarginTop);
								pinchData.lastMarginTop = Math.max(
									-pinchData.diffHeight,
									pinchData.lastMarginTop
								);

								pinchData.lastMarginLeft =
									pinchData.marginLeft + endCoords.pageX - startCoords.pageX;
								pinchData.lastMarginLeft = Math.min(
									0,
									pinchData.lastMarginLeft
								);
								pinchData.lastMarginLeft = Math.max(
									-pinchData.diffWidth,
									pinchData.lastMarginLeft
								);
